...
tf_number = net.importONNXModel(r"C:\Users\calin.tacea\Documents\MATLAB\NeuralNetwork")
...
while True:
    ...
    _, contours, hp = cv2.findContours(imgThresh,1,2)
    ...
    if(area_flag == 1):
        ...
        if detection == 1 and keyboard.is_pressed('p'):
            ...
            neww = w
            newx = x
            newimage = imgThreshCopy[y:y + h, newx:newx + neww]
            newh = h
            ...
            scaled_images = []
            if(len(image_list)==7):
                for cnt in range(len(image_list)-1, -1, -1):
